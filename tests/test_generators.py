...
def test_generated_code_has_no_business_routing_conditions(tmp_path: Path) -> None:
    for platform in ("cloudflare", "docker"):
        result = ProjectGenerator().generate(create_template("web-studio"), tmp_path, platform)
        runtime = next(path for path in result.files if path.name == ("worker.js" if platform == "cloudflare" else "flow.py")).read_text(encoding="utf-8")
        assert "pavel" not in runtime.lower()
        assert "nikolay" not in runtime.lower()
